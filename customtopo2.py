...
#Custom Topology
class MyTopo(Topo):
    def __init__(self, noOFS=4, noHost=1):
        Topo.__init__(self)
        ...
        for i in range(noOFS-1): # "-1" is Linear topology
            ...
            if currOFS != nextOFS:
                self.addLink(currOFS,nextOFS,**linkopts2[i])
...
def main(*args):
    ...
    parser.add_argument('--NumOFS',  type=int, action="store", default=4)
    ...
    print(h1.cmd("sudo ping 10.0.0.4 -f -i 0.1 "))
